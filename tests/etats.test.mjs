...
test('accumulateEtats: doppelte Event-ID wird nicht doppelt gezählt (Idempotenz)', () => {
  ...
  const state = accumulateEtats(events, { ladder: () => 'NARRATIVELY_RELEVANT' });
  assert.equal(state.characters['Squizzle']?.recallCount, 1, 'doppeltes Event zählt nur einmal');
  assert.equal(state.characters['Squizzle']?.knownEvents.length, 1);
});
...
test('accumulateEtats: alle AKTIVEN Characters werden als leere Startzustände angelegt', () => {
