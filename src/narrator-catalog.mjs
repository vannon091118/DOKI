...
//
// RW-100 Narrator-Reduktion (2026-09-06): alle 14 Profile bleiben ARCHIV
// (NARRATORS unverändert, nie löschen). Der AKTIVE Satz wird über
// ACTIVE_NARRATOR_NAMES gesteuert; ensemble-state.mjs materialisiert nur
// noch aktive Characters. Neue aktive Sätze = Namen hier ergänzen (OD4),
// archivierte Profile fallen automatisch aus CHARACTERS/Ensemble.
export const ACTIVE_NARRATOR_NAMES = Object.freeze(['Squizzle', 'Null', 'Argos']);
...
// ── Aktiver Layer (RW-100) ───────────────────────────────────────────────
// fail-closed: ein unbekannter Name in ACTIVE_NARRATOR_NAMES ist ein
// Konfigurationsfehler und wirft (kein stiller Fallback).
const ACTIVE_BY_NAME = new Map(
  ACTIVE_NARRATOR_NAMES.map((name) => {
    const profile = NARRATOR_BY_NAME.get(String(name));
    if (!profile) throw new Error(`ACTIVE_NARRATOR_NAMES references unknown narrator: ${name}`);
    return [String(name), profile];
  })
);

export const ACTIVE_NARRATORS = Object.freeze([...ACTIVE_BY_NAME.values()]);

// ROLE_MAP: die EINZIGE Quelle für Event-Routing-Rollen. Vorher waren
// 'Thinker' als Strings in ensemble-state.mjs, runtime.mjs und
// bridge.mjs verdrahtet — mit dem aktivem Trio wandern die Rollen hierhin
// (P1: keine verteilten Strings). Squizzle erbt die Analyse-/Defense-Rolle
// des archivierten Thinkers.
export const ROLE_MAP = Object.freeze({
  analysis: 'Squizzle',   // war 'Thinker'
  attack: 'Argos',        // evil/evil-twin-Stimme (aggressive Bodenstimme)
  userProxy: 'Vannon',    // User-Stimme (archiviert — nur für historische
                          // Replay-Daten; aktives Routing nutzt sie nicht)
});
...
/** Fail-closed Variante: wirft, wenn der Narrator NICHT aktiv ist. */
export function activeNarratorByName(name) {
  const result = ACTIVE_BY_NAME.get(String(name));
  if (!result) throw new Error(`Narrator is not active (archived): ${name}`);
  return result;
}
...
export function allActiveNarrators() {
  return [...ACTIVE_NARRATORS];
}
