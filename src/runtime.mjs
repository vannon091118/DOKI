...
function deriveEnsembleAndRelevance(snapshot, report) {
  ...
  for (let i = 0; i < findings.length; i++) {
    ...
    candidateBlocks.push({
      ...
      character: isEvil ? ROLE_MAP.attack : ROLE_MAP.analysis,
      target: isEvil ? ROLE_MAP.analysis : null,
    });
  }
  ...
  if (relevantCharacters.length === 0) {
    relevantCharacters.push(ROLE_MAP.attack, ROLE_MAP.analysis);
  }
