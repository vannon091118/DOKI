...
import { narratorByName, NARRATORS, ACTIVE_NARRATORS, ROLE_MAP } from './narrator-catalog.mjs';
import { etats } from './etats.mjs';

// RW-100: Nur AKTIVE Narratoren werden als Ensemble-Characters materialisiert.
// NARRATORS (14) bleibt über narrator-catalog.mjs als Archiv abrufbar
// (allNarrators()); der Zustand trägt nur aktive Identitäten.
export const CHARACTERS = Object.freeze(ACTIVE_NARRATORS.map((n) => n.name));
...
export function accumulateEtats(events = [], rules = {}) {
  // Startzustand: leere characters-Map (alle AKTIVEN Characters mit Leer-State)
  const base = createEmptyEnsemble();
